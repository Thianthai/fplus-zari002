...
| OQ-27 | `payment_amount` ของ header ไม่มีด่านไหนเช็คค่าติดลบเลย — `check_amount_paid_total( )` เช็คแค่ผลรวม `amount_paid` ของ item · ตกลงกันไว้ว่า "ไม่ต้องเช็คเครื่องหมาย" แต่ตอนนั้นยังไม่รู้ว่า trailing minus เข้ามาได้ · จะเพิ่มด่านกันยอดติดลบไหม | ผู้ใช้ + FI | Phase 3 | format check กัน `1070.00-` ได้แล้ว แต่ `"-1070.00"` ยังผ่าน ถ้ายอมรับเครื่องหมายหน้า | ⬜ |
