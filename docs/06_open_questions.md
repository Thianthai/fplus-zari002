...
| OQ-18 | response ของ API — **ปิดแล้ว 2026-09-07** ทุกใบได้ 1 บรรทัดใน `Payments` พร้อม `Status` `S`/`E` รายบรรทัด · ใบที่สำเร็จส่ง `SalesforceId` กลับไปด้วย ผู้เรียกจึงรู้ว่าใบไหนห้ามส่งซ้ำ · ดู `05_api_spec.md` §8 | — | Phase 4 | — | ✅ |
...
| OQ-23 | request ที่มีหลาย payment แล้วบางใบตก — ตอนนี้ใบที่ผ่าน**ถูก commit ไปแล้ว** ใบที่ตกไม่ถูกบันทึก · SBPA ต้องรับสภาพ "request สำเร็จบางส่วน" ได้และส่งเฉพาะใบที่ตกกลับมาใหม่ ไม่ใช่ส่งทั้ง request ซ้ำ (จะติด duplicate ทันที) | SBPA | Phase 5 | ไม่บล็อก — response บอกชัดแล้วว่าใบไหนเข้าไปแล้ว (2026-09-07) เหลือแค่ยืนยันว่า SBPA เอาไปใช้ตามนั้น · ถ้า SBPA คาดหวัง all-or-nothing ต้องรื้อ `process( )` ให้ commit ครั้งเดียวตอนจบ | 🟨 |
...
⚠️ **message `008` ยังค้างอยู่ใน message class** ไม่มีใครเรียกแล้วแต่ยังไม่ได้ลบ
