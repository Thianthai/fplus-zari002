...
| OQ-23 | request ที่มีหลาย payment แล้วบางใบตก — ตอนนี้ใบที่ผ่าน**ถูก commit ไปแล้ว** ใบที่ตกไม่ถูกบันทึก · SFDC ต้องรับสภาพ "request สำเร็จบางส่วน" ได้และส่งเฉพาะใบที่ตกกลับมาใหม่ ไม่ใช่ส่งทั้ง request ซ้ำ (จะติด duplicate ทันที) | Salesforce | Phase 5 | ไม่บล็อก — แต่ถ้า SFDC คาดหวัง all-or-nothing ต้องรื้อ `process( )` ให้ commit ครั้งเดียวตอนจบ | ⬜ |
| OQ-24 | `send_callback( )` ถูกเรียก**ในลูป payment** = 1 request ที่มี 5 payment ยิง callback 5 ครั้ง · ควรเป็นครั้งเดียวต่อ request หรือไม่ | Salesforce | Phase 5 | ผูกกับ OQ-17 — ตอบพร้อมกันตอนได้ contract ของ callback | ⬜ |
