...
| OQ-10 | 🔴 **ตัวเลขที่ผิดรูปไม่มีใครจับ** — พิสูจน์ด้วยการยิงจริง 2026-09-04 · `"1,070.00"` และ `"abc"` → **012 ทั้ง request ตายยกชุด** พร้อม `RequestId`/`SalesforceId`/`Field` ว่างหมด · `"1070.00-"` → **ผ่านทุกด่านแล้วกลายเป็นยอดติดลบ บันทึกลง table** · สาเหตุ: field ตัวเลขถูก type เป็น DDIC ตรง ๆ `xco` แปลงก่อนโค้ดเราเห็นค่าดิบ | Claude + ผู้ใช้ | Phase 3 | **ต้องแก้ก่อน Phase 6** — ไม่ใช่ error message ไม่สวย แต่เป็นข้อมูลการเงินผิดที่ไหลต่อไปถึง ZARE002 | ⬜ กำลังแก้ |
...
| OQ-27 | ยอดติดลบ — **ตกลงแล้ว 2026-09-04: รับได้ เครื่องหมายลบใส่ได้ทั้งหน้าและหลัง** (`-1070.00` และ `1070.00-`) · format check จึงปล่อยผ่านทั้งคู่ ไม่มีด่านกันยอดติดลบที่ระดับ header · ถ้าภายหลัง FI บอกว่าไม่ควรมี ให้เพิ่มด่านแยกโดยใช้ message 011 ที่มีอยู่แล้ว | — | Phase 3 | — | ✅ |
...
### ผลทดสอบตัวเลขผิดรูป — 2026-09-04

ยิงจริงผ่าน Postman บน code ที่ยังไม่ได้แก้ · **ยืนยันว่า SBPA ส่งตัวเลขมาเป็น string จริง**
(`"NumberOfItemsInPayment": "5"`) และ `xco` แปลง string ที่ถูกต้องลง INT4/CURR ได้ไม่มีปัญหา

| ส่งเข้าไป | ได้อะไร |
|---|---|
| `"1,070.00"` | `400` + **012** `Request body is not valid JSON` · `RequestId` `SalesforceId` `Field` ว่างหมด |
| `"abc"` | เหมือนกันทุกอย่าง |
| `"1070.00-"` | ✅ **ผ่าน** → เก็บเป็น `-9650.75` ลง table |

**เคสที่ 3 อันตรายที่สุด** — ไม่มี error ไม่มีสัญญาณเตือน และ Excel ที่ export จากระบบที่ใช้
SAP notation ส่งรูปแบบนี้มาได้ง่ายมากโดยไม่มีใครตั้งใจ · `check_amount_paid_total( )` จับไม่ได้
เพราะมันดูแค่ผลรวม `amount_paid` ของ item ไม่ได้แตะ `payment_amount` ของ header (→ **OQ-27**)

**2 เคสแรกก็หนักกว่าที่คิด** — ผิด field เดียวในใบเดียว ทำให้ทั้ง request ตกยกชุด และ response
ไม่มีข้อมูลพอให้ SBPA รู้ว่าต้องแก้ตรงไหน ต้องไปไล่เองทั้งไฟล์

**เลื่อนงานนี้ขึ้นมาก่อน Phase 6** — เดิมประเมินว่าเป็นเรื่อง error message ไม่สวย ซึ่งประเมินต่ำไป

**แก้กฎ 2026-09-04** — ตอนตั้งโจทย์ระบุว่า "รับเครื่องหมาย + หรือ - ด้านหน้าเท่านั้น"
แต่หลังเห็นผลเทสว่า `1070.00-` เข้ามาได้จริง จึงเปลี่ยนเป็น **รับเครื่องหมายลบทั้งหน้าและหลัง**
· pattern เป็น `^([+-]?\d+(\.\d+)?|\d+(\.\d+)?-)$`

ผลที่ตามมา: trailing minus กลายเป็น **เส้นทางหลักที่ต้องแปลงให้ถูก** ไม่ใช่เคสที่ถูกกันทิ้ง
— `MOVE-CORRESPONDING` string → CURR ต้องอ่าน `1070.00-` เป็น `-1070.00` ได้ ยังไม่เคยพิสูจน์
